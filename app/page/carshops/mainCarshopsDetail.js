...
export default class App extends Component {
    ...
    componentDidMount() {
        ...
        if (this.props.navigation.state && this.props.navigation.state.params.fMaintainPlanId) {
            this.selectDetailByfMaintainPlanId(this.props.navigation.state.params.fMaintainPlanId)
        }
    }
    //查询保养计划详情
    selectDetailByfMaintainPlanId = async (id) => {
        const res = await maintainServer.selectDetailByfMaintainPlanId(id);
        if(res.success){
    ...
    setData = (data) => {
        ...
        for(let item of data.maintainPlanEquipmentResList){
            let ItemIdList = item.fTMaintainItemsAllList.length;
            let fTMaintainItemsAllList = [...item.fTMaintainItemsAllList];
            let num = 0;
            for(let listItems of fTMaintainItemsAllList){
                for(let listItem of item.fMaintenancePlanItemsList){
                    if(listItem.fMaintainItemsId == listItems.fMaintainItemsId){
                        listItems.select = true;
                        num += 1;
                    }
                }
            }
            deviceList.push({
                fId: item.fEquipmentId,
                fEquipmentName: item.fEquipmentName,
                fTMaintainItemsAllList,
                ItemIdList,
                num
            })
        ...
        for(let item of data.maintainPlanUserResList){
        ...
        this.setState({
            fMaintainPlanId: data.fMaintainPlanId,
            workText: data.fMaintainPlanTitle+'',
            date: parseDate(data.tMaintainRule.fNextTime,'YYYY-MM-DD'),
            typeData : {
                fId: data.fMaintainLevel+'',
                fName: this.getCurrentLerver(data.fMaintainLevel),
            },
            deviceList,
            participationArr
        })
    ...
    getValue = async(value) => {
        ...
        if(res.success){
            let fTMaintainItemsAllList = [...res.obj];
            for(let item of fTMaintainItemsAllList){
                item.select = true
            }
            let ItemIdList = fTMaintainItemsAllList.length;
            deviceList.push({
                fId: value.fId,
    ...
    getCarshopItems = (value) => {
        ...
        for(let index in deviceList){
            if(index == this.state.currentIndex){
                let fTMaintainItemsAllList = [...value];
                deviceList[index].fTMaintainItemsAllList = fTMaintainItemsAllList;
    ...
    //更新计划
    updateMaintainTask = async() => {
        let people = [];
        for(let item of this.state.participationArr){
            people.push(item.fId)
        }
        let maintainPlanEquipmentReqList = [];
        for(let item of this.state.deviceList){
            console.log(1111111111,item);
            let fMaintenancePlanItemIdList = []
            item.fTMaintainItemsAllList.map((item) => {
                if(item.select){
        ...
        global.loading.show();
        const res = await maintainServer.updateMaintainTask({
            "fMaintainPlanId": this.state.fMaintainPlanId,
            "fMaintainPlanTitle": this.state.workText,
            "fMaintainTaskDate": parseTime(this.state.date),
            "isIssue": this.state.putCurrent ? 1: 2,
    ...
    //删除任务计划
    maintainPlanDeleteById = async () => {
        const res = await maintainServer.maintainPlanDeleteById(this.props.navigation.state.params.fMaintainPlanId);
        if(res.success){
        ...
        return (
          <View style={styles.container}>
            ...
            <TipModal 
                ...
                onOk={this.maintainPlanDeleteById}
                tipText={`您确定删除当前任务计划吗？`}
            />
            <ScrollView>
                <View style={{paddingLeft: 16,paddingRight: 16,marginTop: 12,backgroundColor: '#FFF'}}>
                    ...
                    <View style={[styles.item, {paddingBottom: 5}]} onPress ={() => {this.showTypeDataList()}}>
                        <View style={{flexDirection: "row",alignItems: "center"}}>
                            ...
                            this.state.deviceList.map((item,index) => {
                                return(<View style={styles.addItem}>    
                                    <View style={{flexDirection: "row",alignItems: "center",flex: 1}}>
                                        ...
                                        <TouchableOpacity onPress={() => {this.getCarshopItem(index,item.fTMaintainItemsAllList)}}>
                    ...
                    <View style={[styles.item,{height: 47}]}>
                        <View style={{flexDirection: "row",alignItems: "center"}}>
                            ...
                            <Text style={{color: "#333333", fontSize: 14,fontWeight: "600",marginLeft: 5}}>立即下发</Text>
                        </View>
                        <Switch
                            style={{width: 30,marginRight: 5}}
                            //动态改变value
                            value={this.state.putCurrent}
                            //当切换开关室回调此方法
                            onValueChange={(value)=>{this.setState({putCurrent: value})}}
                            thumbColor = {this.state.putCurrent?"#5970FE": "#B2B2B2"}
                        />
                    </View>
                </View>
                {/* <View style={{paddingLeft: 16,paddingRight: 16,marginTop: 12,backgroundColor: '#FFF'}}>
                    ...
                    <View style={[styles.anotherItem, {paddingBottom: 5}]}>
                        ...
                        {   
                            ...
                            <View style={{flexDirection: "row",paddingLeft: 10,alignItems: "center"}}>
                                <Text style={styles.buttonText}>每</Text>
                                <View style={{height: 30}}>
                                    <Text>{this.state.monthNum}</Text>
                                </View>  
                                <Text style={styles.buttonText}>月</Text>
                                <View style={{height: 30}}>
                                    <Text>{this.state.monthTime}</Text>
                                </View>  
                        ...
                        {   
                            ...
                            <View style={{flexDirection: "row",paddingLeft: 10,alignItems: "center"}}>
                                <Text style={styles.buttonText}>每</Text>
                                <View style={{height: 30}}>
                                    <Text>{this.state.yearNum}</Text>
                                </View>  
                                <Text style={styles.buttonText}>年</Text>
                                <View style={{height: 30}}>
                                    <Text>{this.state.yearTime}</Text>
                                </View>  
                                <Text style={styles.buttonText}>次</Text>
                            </View>
                            :null
            ...
            <View style={{paddingLeft: 16,paddingRight: 16,flexDirection: "row"}} >
              <TouchableOpacity style={[styles.bottomButton,{marginRight: 10}]} onPress={() => {this.setState({showModals: true})}}>
                  <Text style={{color: "#fff", fontSize: 16}}>删除</Text>
              </TouchableOpacity> 
              <TouchableOpacity style={[styles.bottomButton,{flex: 2}]} onPress={() => {this.updateMaintainTask()}}>
                  <Text style={{color: "#fff", fontSize: 16}}>{this.state.putCurrent? '立即下发':'保存'}</Text>
              </TouchableOpacity> 
